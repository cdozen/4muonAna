                   ...
                   )
{
    ...
    for (int k = 0; k < 10000; k++)
    {
       ...
       if(!Set_Eta) if(!(abs(mu12_p4_vector_mix.Rapidity())> bin1 && abs(mu12_p4_vector_mix.Rapidity())< bin2)) continue;
...
{   
    ...
    TFile *f = new TFile("fourMuMass_treemu34on.root");  //2018 with mu34 mas cut
    ...
    for (int n = 0; n < y_size - 1; n++)
    {
       ...
       //*******************************************************************//
        ...
        for (int i = 0; i < Ntotal; i++)
        {
            ...
            if(!Set_Eta)if (!(abs(ReadTree->Y_upsilon) > ybins[n] && abs(ReadTree->Y_upsilon) < ybins[n + 1]))continue;
            //for eta binning
            if(Set_Eta)if (!(abs(ReadTree->Eta_upsilon) > ybins[n] && abs(ReadTree->Eta_upsilon) < ybins[n + 1]))continue;
            cout << "\r" << "Entry :" << i << "/" << Ntotal << flush;
            //ProduceRandom(ReadTree->mu12,RandomYvector,datasetgen,lxg,i);
            ProduceRandom(ReadTree->mu12, mu12_MixingVect, func, i, ybins[n], ybins[n + 1], Set_Eta);
            // cout<<"finito.."<<endl;
                ...
                
            {
                ...
                mix_Y2Dpteta->Fill(mu12_MixingVect[s].Eta(), mu12_MixingVect[s].Pt());
                mix_Y2Detarapid->Fill(mu12_MixingVect[s].Eta(), mu12_MixingVect[s].Rapidity());
                mix_Y2Dptrapid->Fill(mu12_MixingVect[s].Rapidity(), mu12_MixingVect[s].Pt());
