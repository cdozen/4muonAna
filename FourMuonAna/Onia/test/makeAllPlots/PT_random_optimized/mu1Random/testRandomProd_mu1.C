 ...
 */
/*this code is choose mu3 ( void testRandomProd_mu3(int n)) and mu4 void testRandomProd_mu4(int n) randomlt in different function and then mix them all in the last func. void testRandomProd_mu1()
 */
/* 
 * File:   tester.h
 * Mentor: Zack 
...
//**************************************//
   ...
   // {
        TCut mycuty; // okudugun tree yi bu cut lara gore al.
...
//if kosulunu gecen adamlar geldi 
                        //input,        output,         Tfunc, event num. binning
          ProduceRandom(ReadTree->mu3, mu3_MixingVect, func, i, ybins[n], ybins[n + 1]);
          
          // inputu al , random olarak output uret ve TLorentz vector kutusuna doldur (mu3_MixingVect), 
          // cout<<"finito.."<<endl;
...
//if kosulunu gecen adamlar geldi 
          ...
          // inputu al , random olarak output uret ve TLorentz vector kutusuna doldur (mu4_MixingVect), 
          // cout<<"finito.."<<endl;
...
void testRandomProd_mu1(){
    ...
    testRandomProd_mu3(n);   
    testRandomProd_mu4(n);    
    ...
    {   
        ReadTree->GetEntry(i);
           ...
           Ypt->Fill(ReadTree->mu12->Pt());
            ...
            //hfourMuFitmu4Eta->Fill(ReadTree->m34->Eta());
            //hfourMuFitmu4Rap->Fill(ReadTree->m34->Rapidity());
...
//float muonPtCut[5]={2,2.5,3,3.5,4};
        ...
        //for(int j= 0; j< mu4_MixingVect.size(); j++ )
        {
          //  for(int k= 0; k< mu3_MixingVect.size(); k++ )
            //{
                
           if (!(abs(mu3_MixingVect[j].Eta())-abs((*ReadTree->mu3).Eta()) > 0.1 && abs(mu4_MixingVect[i].Eta())- abs((*ReadTree->mu4).Eta())>0.1))continue;
           //if (!(abs(mu3_MixingVect.Eta()) > ybins[n] && abs(mu3_MixingVect.Eta()) < ybins[n + 1]))continue;
           //if (!(abs(mu4_MixingVect.Eta()) > ybins[n] && abs(mu4_MixingVect.Eta()) < ybins[n + 1]))continue;
                
                
                TLorentzVector mu34Mixing = mu4_MixingVect[j] + mu3_MixingVect[j];

           //if (!(abs(mu34Mixing.Eta()) > ybins[n] && abs(mu34Mixing.Eta()) < ybins[n + 1]))continue;
...
//***********************************************************//  
    TString Cname = "Graph_total_formumix";
