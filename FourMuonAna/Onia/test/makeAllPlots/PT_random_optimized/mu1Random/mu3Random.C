...
//**************************************//
    ...
    {
        TCut mycuty;
            mycuty = Form("(abs(%s) > %.2f) && (abs(%s) < %.2f)", "mu3_eta", ybins[n], "mu3_eta", ybins[n + 1]);
        ...
        TString totalbin;
            totalbin = Form("%s_%.2f_%.2f", "mu3_eta", ybins[n], ybins[n + 1]);
...
//*******************************************************************//      
        ...
        RooFFTConvPdf *lxg=new RooFFTConvPdf ("lxg","landau (X) gauss",*var_mu3_pt,landau,gauss) ;
       ...
       //*******************************************************************//
        ...
        lxg->fitTo(*dataset, RooFit::NumCPU(4, RooFit::BulkPartition));
         ...
         // Set func for error from Muhammad
          TF1 *func ;
          func = lxg->asTF(RooArgList(*var_mu3_pt));
        


        // Plot data, landau pdf, landau (X) gauss pdf
            GraphTitle = lxg->GetName();
            GraphTitle += "_mu3_EtaBin";
        
        RooPlot* frame = var_mu3_pt->frame(Title(GraphTitle + totalbin));
...
//****************************//
        ...
        for (int i = 0; i < Ntotal; i++)
        {
            ...
            if (!(abs(ReadTree->mu3_eta) > ybins[n] && abs(ReadTree->mu3_eta) < ybins[n + 1]))continue;
            
            cout << "\r" << "Entry :" << i << "/" << Ntotal << flush;
            
            //ProduceRandom(ReadTree->mu12,RandomYvector,datasetgen,lxg,i);
            ProduceRandom(ReadTree->mu3, mu3_MixingVect, func, i, ybins[n], ybins[n + 1]);
            // cout<<"finito.."<<endl;
...
///*******************************//
            ...
            hfourMuFitmu3Pt->Fill(ReadTree->mu3->Pt());
            hfourMuFitmu3Eta->Fill(ReadTree->mu3->Eta());
            hfourMuFitmu3Rap->Fill(ReadTree->mu3->Rapidity());
            ...
            //mix fourMuon
            for (auto s = 0; s < mu3_MixingVect.size(); s++)
                
            {
                TLorentzVector mu3Mixing= *ReadTree->mu4 + mu3_MixingVect[s];
                mixFourMu = *ReadTree->mu12 + mu3Mixing;
                mix_Ypt->Fill(mu3Mixing.Pt());
                mix_Yeta->Fill(mu3Mixing.Eta());
                mix_Yrapid->Fill(mu3Mixing.Rapidity());
                mix_Yphi->Fill(mu3Mixing.Phi());
                mix_Y2Dpteta->Fill(mu3Mixing.Eta(), mu3Mixing.Pt());
                mix_Y2Detarapid->Fill(mu3Mixing.Eta(), mu3Mixing.Rapidity());
                mix_Y2Dptrapid->Fill(mu3Mixing.Rapidity(), mu3Mixing.Pt());
                ...
                h_mix_fourMuFitmu3Pt->Fill(mu3_MixingVect[s].Pt());
                h_mix_fourMuFitmu3Eta->Fill(mu3_MixingVect[s].Eta());
                h_mix_fourMuFitmu3Rap->Fill(mu3_MixingVect[s].Rapidity());
            }
            //
...
//***********************************************************//  
    ...
    Y2Dpteta->Draw("colz");
    c2->SaveAs("plots2D_mu3/Y2Dpteta.png");
    mix_Y2Dpteta->Draw("colz");
    c2->SaveAs("plots2D_mu3/Y2Dpteta_mix.png");
    Y2Dptrapid->Draw("colz");
    c2->SaveAs("plots2D_mu3/Y2Dptrapidity.png");
    Y2Detarapid->Draw("colz");
    c2->SaveAs("plots2D_mu3/Y2Detarapidity.png");
    mix_Y2Dptrapid->Draw("colz");
    c2->SaveAs("plots2D_mu3/Y2Dptrapidity_mix.png");
    ...
    c2->SaveAs("plots_mu3/hfourMuMass_origvsmix.png");
   ...
   * hfourMuMass_aftercut_smallrange->SetBinContent(32,0);*/
    ...
    c2->SaveAs("plots_mu3/hfourMuMass_afterCut_origvsmix.png");
    ...
    c2->SaveAs("plots_mu3/hfourMuMass_physkbg_mix.png");
    ...
    c2->SaveAs("plots_mu3/hfourMuMass_physkbg_mix_smallrange.png");
    ...
    leg->Draw("same");
    c3->SaveAs("plots_mu3/hfourMuMass_afterCut_origVSphyskbg.png");
    ...
    leg->Draw("same");
    c3->SaveAs("plots_mu3/hfourMuMass_afterCut_origVSphyskbg_smallrange.png");
