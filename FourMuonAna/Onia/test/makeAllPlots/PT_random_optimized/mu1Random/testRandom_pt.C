                   ...
                   )
{
    ...
    for (int k = 0; k < 10000; k++)
    {
        ...
        if( !(abs(temp_vector_mix1.Eta())>bin1 && abs(inputvector1->Eta()) < bin2)) continue;
        if( !(abs(temp_vector_mix2.Eta())>bin1 && abs(inputvector2->Eta()) < bin2)) continue;
...
//****************************//
    ...
    vector<TLorentzVector> mu4_MixingVect;
        ...
        for (int i = 0; i < Ntotal; i++)
        {
            ...
            ProduceRandom(ReadTree->mu3,ReadTree->mu4, mu34_MixingVect, func1,func2, i, ybins[n], ybins[n + 1]);
...

        {
           ...
           //if (!(abs(mu3_MixingVect[j].Eta()) > ybins[n] && abs(mu3_MixingVect[j].Eta()) < ybins[n + 1]))continue;
           //if (!(abs(mu4_MixingVect[j].Eta()) > ybins[n] && abs(mu4_MixingVect[j].Eta()) < ybins[n + 1]))continue;
...
//if( abs(mu34_MixingVect[j].Eta())- abs((*ReadTree->mu34).Eta()) > 0.1) continue;
